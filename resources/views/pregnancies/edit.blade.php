...
@section('title', 'Edit Pregnancy Record')

@section('content')
    <div class="card">
        <div class="card-header">
            <h3>Edit Pregnancy Record for {{ $pregnancy->patient->name }}</h3>
        </div>
        <div class="card-body">
            <form action="{{ route('pregnancies.update', $pregnancy) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    ...
                    <input type="date" class="form-control @error('first_day_of_last_period') is-invalid @enderror"
                        ...
                        value="{{ old('first_day_of_last_period', $pregnancy->first_day_of_last_period) }}" required>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('pregnancy_count') is-invalid @enderror"
                        id="pregnancy_count" name="pregnancy_count"
                        value="{{ old('pregnancy_count', $pregnancy->pregnancy_count) }}" min="1" required>
                ...
                <div class="mb-3">
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" id="is_high_risk" name="is_high_risk" value="1"
                            {{ old('is_high_risk', $pregnancy->is_high_risk) ? 'checked' : '' }}>
                ...
                <div class="mb-3">
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" id="is_active" name="is_active" value="1"
                            {{ old('is_active', $pregnancy->is_active) ? 'checked' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('risk_notes') is-invalid @enderror" id="risk_notes" name="risk_notes"
                        rows="3">{{ old('risk_notes', $pregnancy->risk_notes) }}</textarea>
                    @error('risk_notes')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="d-flex justify-content-between">
                    <a href="{{ route('pregnancies.show', $pregnancy) }}" class="btn btn-secondary">Back</a>
                    <button type="submit" class="btn btn-primary">Update Pregnancy Record</button>
                </div>
            </form>
        </div>
    </div>
